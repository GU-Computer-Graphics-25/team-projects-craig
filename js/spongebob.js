export function createSpongebob() {
    ...
    // helper builds an arm, snaps a hand to its far end, and returns both
    function buildArm(side = 'L') {
    const arm = new THREE.Mesh(armGeometry, legArmMaterial);

    /* Shoulder position */
    const x = (side === 'L' ? -1 : 1) * (bodyWidth / 2.27);
    arm.position.set(x, bodyMesh.position.y - 0.6, .7);

    arm.rotation.set(Math.PI / 2, Math.PI, Math.PI);     
    ...
    hand.position.set(0, -armLength / 2, 0);   // “-Y” end of the cylinder
    arm.add(hand);

    return { arm, hand };
    }

    const { arm: leftArmMesh  } = buildArm('L');
    const { arm: rightArmMesh, hand: rightHand } = buildArm('R');

    /* Add a pivot group to the left arm */
    const leftShoulderGroup = new THREE.Object3D();
    leftShoulderGroup.position.set(
    rightArmMesh.position.x,
    rightArmMesh.position.y,
    rightArmMesh.position.z
    ); 
    leftShoulderGroup.rotation.set(Math.PI / 2, Math.PI, Math.PI); // Match arm rotation
    spongebobGroup.add(leftShoulderGroup);

    rightArmMesh.position.set(0, 0, 0);
    leftShoulderGroup.add(rightArmMesh);
    ...
    gui.add(parm, 'angleDeg', 0, 80, 1)           // limits: tweak later
    .name('Flip arm');                         // shows in the GUI
